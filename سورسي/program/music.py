...
async def play(c: Client, m: Message):
    ...
    if do.count("left") or do.count("Bad Request: user not found"):
        ...
        try:
            ...
            if b.status == "kicked":
                await m.reply_text(
                    f"@{ASSISTANT_NAME} **محظور فّـې القروب** {m.chat.title}\n\n» **قم بفك حظرأّلَمَ ستّخَدِمَ أوِلَأًّ إوِ تّأّګدِ مَنِ تّقِيِّدِ حٌ سأّبِ أّلَمَ سأّعٌد.**"
                )
                return
        ...
        if replied:
            if replied.audio or replied.voice:
                suhu = await replied.reply("**أبشر ياعيني ثواެني بس اެبحث🦦 .**")
                    ...
                    if search == 0:
                        ...
                        if veez == 0:
                            ...
                            if chat_id in QUEUE:
                                ...
                                try:
                                    await suhu.edit("🔄 ** تم الانظمام للمكالمة 🦦والتشغيل...**")
                                  await call_py.join_group_call(
                                        chat_id,
                                        AudioPiped(
                                            ytlink,
                                    ...
                                    await suhu.delete()
                                    await m.reply_text(f"🚫 حدث خَطّأ تّأّګدِ مَنِ أّلَمَګأّلَمَهِ مَفِّتّوِحٌهِ  أّوِلَآ: `{ep}`")
                ...
                if search == 0:
                    ...
                    if veez == 0:
                        ...
                        if chat_id in QUEUE:
                            ...
                            await m.reply_photo(
                                photo=thumbnail,
                                caption=f"🦦 ***-› ابشر ياعيني ضفتها للانتظار** `{pos}`\n\n🏷 **-› اެݪاެسم:** [{songname}]({url})| موسيقى`\n**⏱ أّلَمَدِ ةّ:** `{duration}`\n🎧 **-› طݪب اެݪحݪۅٛ:** {requester}",
                                reply_markup=keyboard,
                            )
                        else:
                            try:
                                await suhu.edit("🔄 **الانظمام إلى المكالمة...**")
                                await call_py.join_group_call(
                                    chat_id,
                                    AudioPiped(
                                        ytlink,
                                ...
                                await suhu.delete()
                                await m.reply_text(f"🚫حدث خطأ تاكد ﻣ̝نّ المكالمة مفتوحه او لا`{ep}`")
