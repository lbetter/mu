...
@Client.on_message(
    command(["userbotjoin", f"بيتر ادخل", "اطربهم يابيتر"]) & other_filters
)
...
async def join_chat(c: Client, m: Message):
    ...
    try:
        ...
        if invitelink.startswith("https://t.me/+"):
            ...
            return await user.send_message(chat_id, "انضممت هنا وبطربهم على  طلبك🦦")
    except UserAlreadyParticipant:
        return await user.send_message(chat_id, "انا بالفعل موجود هنا 🦦")


@Client.on_message(
    command(["userbotleave", f"بيتر اطلع", "غادر "]) & other_filters
)
@authorized_users_only
